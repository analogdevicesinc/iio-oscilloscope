...
static int cf_axi_tdd_v2_chann_widgets_init(struct plugin_private *priv, struct iio_device *dev)
{
	const unsigned int n_channels = iio_device_get_channels_count(dev);
	struct iio_channel *chan;
	unsigned int i;
				  ...
				  const char *ini_fn)
{
	...
	if(!iio_device_find_attr(dev, "version")) {
	    ...
	    iio_spin_button_int_init_from_builder(&priv->w[priv->n_w++], dev, NULL, "frame_length_ms",
		priv->builder, "frame_length", NULL);
	    iio_spin_button_int_init_from_builder(&priv->w[priv->n_w++], dev, NULL, "internal_sync_period_ms",
		priv->builder, "int_sync_period", NULL);
	    ...
	    iio_toggle_button_init_from_builder(&priv->w[priv->n_w++], dev, NULL, "sync_internal",
		priv->builder, "internal_sync_enable", false);
	    iio_button_init_from_builder(&priv->w[priv->n_w++], dev, NULL, "sync_reset",
		priv->builder, "reset_sync_button");
	    iio_button_init_from_builder(&priv->w[priv->n_w++], dev, NULL, "sync_soft",
		priv->builder, "soft_sync_button");

	    /* init channel widgets */
	    ret = cf_axi_tdd_v2_chann_widgets_init(priv, dev);
	    if (ret)
	      goto context_destroy;
