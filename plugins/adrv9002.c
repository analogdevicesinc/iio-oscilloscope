...
static void adrv9002_check_tx_track_support(struct plugin_private *priv, struct adrv9002_common *tx)
{
	...
	for (w = 0; w < tx->num_widgets; w++) {
		...
		if (!checked) {
			ret = chn_attr_read_raw(tx->w[w].chn, tx->w[w].attr_name, dummy, sizeof(dummy));
			checked = true;
		}
...
static void adrv9002_update_port_en_mode(const struct plugin_private *priv, struct adrv9002_common *chan)
{
	...
	if (!strcmp(port_en, "pin")) {
		 ...
		 */
		ret = chn_attr_write_string(chan->port_en.chn, "port_en_mode", "pin");
		if (ret < 0) {
	...
	ret = chn_attr_read_raw(chan->ensm.chn, "ensm_mode", pin_ensm, sizeof(pin_ensm));
	if (ret < 0)
		goto out;
	 ...
	 */
	ret = chn_attr_write_string(chan->ensm.chn, "ensm_mode", "primed");
	if (ret > 0)
	 ...
	 */
	if (chan->enable_gpio) {
		chn_attr_write_string(chan->ensm.chn, "ensm_mode", pin_ensm);
		if (!strcmp(port_en, "pin"))
	 ...
	 */
	if (strcmp(port_en, "pin"))
		chn_attr_write_string(chan->port_en.chn, "port_en_mode", "spi");
